...
# -------------------------
# 4. Simple Fact Checking via Wikipedia
# -------------------------
with col2:
    if st.button("🔎 Fact Check (Wikipedia)"):
        if news_input.strip() == "":
            ...
            try:
                ...
                summary = wikipedia.summary(subject, sentences=2).lower()
                ...
                if any(word in news_input.lower() for word in sensitive_words):
                    ...
                    if matched > len(words) * 0.3:
                        st.success("✅ Likely TRUE (matches Wikipedia)")
                    else:
                        st.error("❌ Possibly FALSE (not confirmed on Wikipedia)")
